        ...
        try {
            ...
            if (list != null && list.size() > 0) {
                ...
                // 机构选择
                    ...
                    sb.append("<input type='hidden' data-seq='" + WebTools.getOrganLevel(user.getOrganCode(), defValue, currOrganId) + "' value='"
                        + defValue + "' id='" + ((String) list.get(3)) + "' name='" + (String) list.get(4) + "'>");
                    if ("ALL".equals(user.getOrganCode())) {
                        // 不显示当前机构
                    } else {
                        ...
                        sb.append("<select " + ((String) list.get(6)) + " data-parent='1' data-first='1' onchange='organ.change(this);' >")
                            .append("<option value=''>请选择</option>").append("<option selected value='" + (flag ? currOrganId : user.getOrganId())
                                + "'>" + dao.getOrganByOrganId(map1).get("ABBR_NAME") + "</option>")
                            .append("</select>");
                    }
                    if (organList != null && organList.size() > 0) {
                        sb.append("<select " + ((String) list.get(6)) + " data-parent='1' "
                            + ("ALL".equals(user.getOrganCode()) ? " data-first='1' " : "") + " onchange='organ.change(this);'>");
                ...
                // 来源级联下拉列表
                else if ("fromtype".equalsIgnoreCase(tag)) {
                    String showLevel = (String) list.get(1);
                    String defValue = (String) list.get(2);
                    Map<String, Object> map = new HashMap<>();
                    if (StringUtils.isBlank(defValue)||"0".equals(defValue)||"10".equals(defValue)) {
                        map.put("codeLevel", "1");
                        List<Map<String, Object>> result = dao.getFromtype(map);
                        sb.append("<input type='hidden' name='code' value=''>");
                        sb.append("<select level='1'");
                        sb.append(((String) list.get(6)));// 添加属性
                        sb.append(" onchange='fromtype.change(this)'");// onchange事件
                        sb.append(">");
                        sb.append("<option value=''>请选择</option>");
                        for (Map<String, Object> obj : result) {
                            sb.append("<option value='" + obj.get("CODE") + "' >");
                            sb.append(obj.get("NAME") + "</option>");
                        }
                        sb.append("</select>");
                    } else {
                        map.put("code", defValue);
                        List<Map<String, Object>> result = dao.getFromtypePid(map);
                        sb.append("<input type='hidden' name='code' value='"+defValue+"'>");
                        for(Map<String, Object> obj:result){
                            map.clear();
                            map.put("pid", obj.get("PID"));
                            List<Map<String, Object>> res = dao.getFromtype(map);
                            if(res.size()>0){
                                sb.append("<select onchange='fromtype.change(this)' level='"+res.get(0).get("CODELEVEL")+"'");
                                sb.append(((String) list.get(6)));// 添加属性
                                sb.append(">");
                                sb.append("<option value='"+obj.get("PID")+"'>请选择</option>");
                            }
                            for(Map<String, Object> ob:res){
                                sb.append("<option ");
                                sb.append(obj.get("CODE").toString().equals(ob.get("CODE"))?"selected":"");
                                sb.append(" value='"+ob.get("CODE").toString()+"'>");
                                sb.append(ob.get("NAME").toString()+"</option>");
                            }
                            sb.append("</select>");
