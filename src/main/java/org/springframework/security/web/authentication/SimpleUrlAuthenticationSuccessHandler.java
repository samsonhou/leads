...
public class SimpleUrlAuthenticationSuccessHandler extends AbstractAuthenticationTargetUrlRequestHandler implements AuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
        ...
        if(authentication!=null && authentication.isAuthenticated()){
        	AuthorUser user=(AuthorUser) authentication.getPrincipal();
        	user.setLoginIp(WebTools.getIpAddr(request));
        	UserLoginLog.updateLoginIp(WebTools.getIpAddr(request),user.getUuid());
        }
